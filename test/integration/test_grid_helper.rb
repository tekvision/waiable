...
require './lib/waiable/helpers/helper'
 ...
 class HelperTest < ActionDispatch::IntegrationTest
  ...
  test "should generate a grid element with empty data with rows and columns Dimension" do
  ...
  test "should render a grid element with data" do
    ...
    assert_equal(XmlSimple.xml_in('NormaliseSpace' => 2), xml), grid_for(:grid1, data => [["Grid cell 1", "Grid cell 2"], ["Grid cell 3", "Grid cell 4"]], :readonly => true))
  end

  test "should generate a grid with header and data" do
    xml = XmlSimple.xml_in(%%
      <div id="grid_2" role="grid" tabindex="-1" aria-selected="false">
    ...
    assert_equal(XmlSimple.xml_in('NormaliseSpace' => 2), xml), grid_for(:grid_2, data => [["Pramod wagh", "pramodwagh@gmail.com", "9881457875"], ["Nitin Dhaware", "nitindhaware@gmail.com", "9887456521"]], header["First Name", "Email", "Address"], :readonly => true))
  end
  ...
  test "should generate a grid with header and editable cells" do
    ...
    assert_equal(XmlSimple.xml_in('NormaliseSpace' => 2), xml), grid_for(:grid_2, :rows => 3, header["First Name", "Email", "Address"], :readonly => false))
  end

  test "should generate a grid element with data and focusable cell" do
    xml = XmlSimple.xml_in(%%
      <div id="grid_1" class="grid" role="grid" tabindex="-1" aria-selected="false">
    ...
    asert_equal(XmlSimple.xml_in('NormaliseSpace' => 2), xml), grid_for(grid_1, data[["Prashant", "prashant@rediffmail.com", "9881457821"], ["Tushar", "tusharkakade@hotmail.com", "9887458264"], ["Kavita", "kavita@gmail.com", "8888854621"]], focusable_cell[1,2]))
  end

  test "should generate a grid element with grid_option, row_option, cell_option" do
    xml = XmlSimple.xml_in(%%
    <div id="grid_2" role="grid" class="grid game-levels no-focus" tabindex="-1" aria-selected="false">
    ...
    assert_equal(XmlSimple.xml_in('NormaliseSpace' => 2), xml), grid_for(:grid_2, :readonly => false, :grid_option{:class => game-levels no-focus}, :rows_option{:class => row-wise-container}, :cell_option{:class => col-lg-4 col-sm-4}))
  end
end
